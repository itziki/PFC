...
public class minimax extends java.applet.Applet implements ActionListener{
	Panel p,principal;
	Button bNuevo;
	Button casilla[][];
	Image cero, equis;
	...
	public void paint(Graphics g){
		...
		if(quienGana>0){
			...
			if(quienGana==1) pos.setText("¡TU GANAS!");
			else if(quienGana==2) pos.setText("¡YO GANO!");
		}
	}
	...
	public void actualiza(){
		for(int i=0;i<3;i++)
			for(int j=0;j<3;j++)
				if (tablero[i][j]==1) casilla[i][j].setLabel("O");
				else if (tablero[i][j]==-1) casilla[i][j].setLabel(":-)");	
				else casilla[i][j].setLabel("");
	...
	public boolean minMax() {
		...
		pos.setText("Posición optima (ren,col): ("+(save_row+1)+", "+(save_col+1)+")");
		repaint();
		return estaLleno();
	} 
